...
- P0에서는 공개 가입 없이 배포 환경에 설정한 관리자 세션만 운영 기능을 사용하며, 관리자 UI와 API는 인증된 `ADMIN`에게만 노출·허용한다. 초대·승인과 OIDC는 후속 PoC 범위다.
...
| 관리자 계정 | P0는 공개 가입 없는 환경 설정 관리자 세션만 허용하고, 초대·승인과 OIDC는 후속 PoC에서 확정 |
