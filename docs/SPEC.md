...
- 사용자는 복수 카테고리, 검색어, 보정 사전으로 매장을 찾고 상세에서 주소·영업시간·연락처·길찾기를 확인한다.
- 매장 정보는 검수/정정 상태와 최신성 기준을 가진다.
- 크롤링과 NAVER 지도·검색 관련 API를 후보 수집 채널로 검증하고, 배치 수집한 후보를 중복 제거·검수한 뒤 공개한다. 공급자별 사용 가능 범위와 정책은 PoC에서 확정한다.
- 피드백과 관리자 운영은 P0에 포함하며, 관리자 UI와 API는 인증된 `ADMIN`에게만 노출·허용한다.
...
아래 항목은 아직 규범적인 동작이 확정되지 않았다. 결정 전에는 구현에서 임의로 해석하지 않는다.

| ID | P0 결정 항목 |
| --- | --- |
| OPEN-P0-01 | 복수 카테고리를 OR 또는 AND 중 어떤 방식으로 결합할지 |
| OPEN-P0-02 | 검색어 보정 후 사용자가 원 검색어 결과로 전환할 수 있어야 하는지 |
| OPEN-P0-03 | 외부 수집 채널별 허용 범위, 중복 판정 기준, 배치 주기와 검증 만료 기간 |
| OPEN-P0-04 | 실제 관리자 OIDC 공급자, 프런트엔드 세션 방식과 `ADMIN` UI 노출 검증 기준 |
| OPEN-P0-05 | 전국 범위 첫 출시를 허용할 최소 지역·카테고리별 데이터 기준 |
...
| API-CUR-01 | API 계약 | `GET /stores`, `GET /stores/{storeId}`가 복수 카테고리를 포함한 지도/목록/상세 조회 경계를 제공한다. |
...
| INT-CUR-01 | 통합 | 동일 필터가 지도와 목록에 같은 검수 매장 집합을 제공하고 상세로 이어진다. |
...
| UI-CUR-01 | UI | 지도, 목록, 상세, 길찾기, 목록 대체 경로가 접근 가능하게 동작한다. |
...
| E2E-CUR-01 | E2E | 사용자가 검색/카테고리에서 매장 상세와 외부 길찾기까지 완료한다. |
