...
class CatalogWorkflowIntegrationTest @Autowired constructor(
    ...
    fun resetMutableData() {
        ...
        val seedIds = listOf(UUID.fromString("11111111-1111-1111-1111-111111111111"), UUID.fromString("22222222-2222-2222-2222-222222222222"))
        jdbc.update("DELETE FROM correction_rules WHERE id <> :seed", mapOf("seed" to UUID.fromString("33333333-3333-3333-3333-333333333333")))
    ...
    fun `admin boundary fails closed without OIDC configuration`() {
        ...
        mockMvc.patch("/api/v1/admin/feedback/11111111-1111-1111-1111-111111111111") {
    ...
    fun `public discovery filters published stores and discloses corrections`() {
        val draft = catalog.create(
            StoreUpsertCommand("Hidden draft", "Seoul", Coordinates(BigDecimal("37.5"), BigDecimal("127.0")), setOf(Category.BACKPACKING)),
            "test-admin",
        )
        val published = catalog.create(
            StoreUpsertCommand("Backpacking House", "Seoul", Coordinates(BigDecimal("37.57"), BigDecimal("126.98")), setOf(Category.BACKPACKING)),
            "test-admin",
        )
    ...
    fun `admin operations expose protected queue health rules and resolution state`() {
        val feedbackId = catalog.submitFeedback(FeedbackSubmitCommand(FeedbackKind.GENERAL, content = "Please update hours", replyEmail = "admin@example.com", contactConsent = true)).id
        ...
        mockMvc.patch("/api/v1/admin/feedback/$feedbackId") {
            with(admin)
            contentType = org.springframework.http.MediaType.APPLICATION_JSON
    ...
    fun `admin can update and delete correction rules with audit events`() {
        ...
        mockMvc.patch("/api/v1/admin/correction-rules/${rule.id}") {
            with(admin)
            contentType = org.springframework.http.MediaType.APPLICATION_JSON
        ...
        mockMvc.delete("/api/v1/admin/correction-rules/${rule.id}") { with(admin) }
            .andExpect {
                status { isOk() }
        ...
        assertEquals(listOf("UPDATE_CORRECTION_RULE", "DELETE_CORRECTION_RULE"), catalog.auditEvents(rule.id).map { it.action }.takeLast(2))
    }
    ...
    fun `feedback resolution records notification state only for consented contact`() {
        val receipt = catalog.submitFeedback(FeedbackSubmitCommand(FeedbackKind.GENERAL, content = "Please update hours", replyEmail = "admin@example.test", contactConsent = true))

        val resolved = catalog.resolveFeedback(receipt.id, FeedbackResolveCommand(FeedbackResolutionStatus.RESOLVED, "Hours verified"), "test-admin")

        assertEquals(FeedbackResolutionStatus.RESOLVED, resolved?.resolutionStatus)
        assertEquals(NotificationStatus.QUEUED, resolved?.notificationStatus)
        assertEquals("Hours verified", resolved?.resolutionSummary)
        assertEquals(1, catalog.notificationAttempts(receipt.id))
        assertEquals(null, catalog.resolveFeedback(receipt.id, FeedbackResolveCommand(FeedbackResolutionStatus.REJECTED, "Already handled"), "test-admin"))
        assertEquals(1, catalog.notificationAttempts(receipt.id))
        assertEquals("RESOLVED_FEEDBACK", catalog.auditEvents(receipt.id).last().action)
    ...
    fun `feedback resolution rejects pending or blank input without audit`() {
        ...
        val admin = TestAuthentication.admin()
        mockMvc.patch("/api/v1/admin/feedback/${receipt.id}") {
            with(admin)
            contentType = org.springframework.http.MediaType.APPLICATION_JSON
    ...
    fun `category health and flags are protected and auditable`() {
        ...
        val admin = TestAuthentication.admin()
        mockMvc.post("/api/v1/admin/category-review-flags") {
            with(admin)
            contentType = org.springframework.http.MediaType.APPLICATION_JSON
        ...
        mockMvc.patch("/api/v1/admin/category-review-flags/${flag.id}") {
            with(admin)
            contentType = org.springframework.http.MediaType.APPLICATION_JSON
            content = """{"state":"RESOLVED"}"""
        }.andExpect { status { isOk() }; jsonPath("$.data.state") { value("RESOLVED") } }
    ...
    fun `manual category flags require store and bounded text`() {
        ...
        val flag = catalog.createManualCategoryReviewFlag(ManualCategoryReviewFlagCommand(store.id, "Category needs review"), "test-admin")
        mockMvc.patch("/api/v1/admin/category-review-flags/${flag.id}") {
            with(admin)
            contentType = org.springframework.http.MediaType.APPLICATION_JSON
    ...
    fun `published stores cannot be edited directly`() {
        ...
        mockMvc.patch("/api/v1/admin/stores/11111111-1111-1111-1111-111111111111") {
            with(admin)
            contentType = org.springframework.http.MediaType.APPLICATION_JSON
            content = """{"name":"Changed public store","address":"Seoul","coordinates":{"latitude":37.5,"longitude":127.0},"categories":["HIKING"]}"""
        }.andExpect {
            status { isConflict() }
            jsonPath("$.success") { value(false) }
    ...
    fun `category flags require explicit feedback input and can be raised from rejection reasons`() {
        ...
        val draft = catalog.create(
            StoreUpsertCommand("Review store", "Seoul", Coordinates(BigDecimal("37.5"), BigDecimal("127.0")), setOf(Category.HIKING)),
            "test-admin",
        )
        catalog.transition(draft.id, StoreStatus.IN_REVIEW, "test-admin")
