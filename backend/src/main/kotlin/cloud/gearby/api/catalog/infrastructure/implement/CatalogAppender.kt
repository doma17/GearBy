...
class CatalogAppender(
    ...
    fun appendNotificationAttempt(feedbackId: UUID, recipientEmail: String, actor: String) {
        // The unique attempt check keeps feedback resolution retry-safe.
        if (!attempts.existsByFeedbackId(feedbackId)) {
            attempts.save(FeedbackNotificationAttemptEntity(feedbackId = feedbackId, recipientEmail = recipientEmail).apply { createdBy(actor) })
        }
    }
    fun audit(action: String, id: UUID, actor: String, before: String?, after: String?, resourceType: String = "STORE") {
        // Persist scalar snapshots as JSON strings so audit columns remain valid JSONB values.
        audits.save(AuditEventEntity(actor = actor, action = action, resourceType = resourceType, resourceId = id, beforeState = before?.let(::jsonQuote), afterState = after?.let(::jsonQuote)).apply { createdBy(actor) })
    }
