...
@Service
class CatalogService(private val reader: CatalogReader, private val manager: CatalogManager) {
    ...
    fun search(query: StoreQuery): StorePageResult {
        ...
        query.bbox?.let { box -> stores = stores.filter { it.coordinates.longitude in box.minLongitude..box.maxLongitude && it.coordinates.latitude in box.minLatitude..box.maxLatitude } }
        if (original != null) {
            ...
            if (direct.isNotEmpty()) stores = direct else reader.correctionFor(normalize(original))?.let { rule ->
                applied = rule.target
                correction = "$original → ${rule.target}"
                stores = when (rule.targetType) {
        ...
        stores = if (query.sort == "distance") stores.sortedWith(compareBy<StoreResult> { it.distanceTo(query.near!!) }.thenBy { it.name }.thenBy { it.id })
        else stores.sortedWith(compareBy<StoreResult> { it.name }.thenBy { it.id })
    ...
    @Transactional fun create(command: StoreUpsertCommand, actor: String): StoreResult = manager.createStore(command, actor).toResult()
    @Transactional(noRollbackFor = [IllegalArgumentException::class]) fun update(id: UUID, command: StoreUpsertCommand, actor: String): StoreResult? = manager.updateStore(id, command, actor)?.toResult()
    @Transactional fun transition(id: UUID, target: StoreStatus, actor: String, reason: String? = null): StoreResult? = manager.transitionStore(id, target, actor, reason)?.toResult()
    @Transactional fun submitFeedback(command: FeedbackSubmitCommand): FeedbackReceiptResult = FeedbackReceiptResult(manager.submitFeedback(command).id)
    @Transactional fun resolveFeedback(id: UUID, command: FeedbackResolveCommand, actor: String): AdminFeedbackResult? = manager.resolveFeedback(id, command, actor)?.let { feedback().first { result -> result.id == it.id } }
    @Transactional fun createCorrectionRule(command: CorrectionRuleCommand, actor: String = "system"): AdminCorrectionRuleResult = manager.createCorrectionRule(command, actor).toResult()
    @Transactional fun updateCorrectionRule(id: UUID, command: CorrectionRuleCommand, actor: String): AdminCorrectionRuleResult? = manager.updateCorrectionRule(id, command, actor)?.toResult()
    @Transactional fun deleteCorrectionRule(id: UUID, actor: String): Boolean = manager.deleteCorrectionRule(id, actor)
    @Transactional fun createManualCategoryReviewFlag(command: ManualCategoryReviewFlagCommand, actor: String): CategoryReviewFlagResult = manager.createManualCategoryReviewFlag(command, actor).toResult(reader.findStore(command.storeId!!)?.name)
    @Transactional fun updateCategoryReviewFlag(id: UUID, command: CategoryReviewFlagUpdateCommand, actor: String): CategoryReviewFlagResult? = manager.updateCategoryReviewFlag(id, command, actor)?.let { it.toResult(reader.findStore(it.storeId)?.name) }

    fun categoryReviewFlags(state: CategoryReviewFlagState? = null, storeId: UUID? = null, assignee: String? = null): List<CategoryReviewFlagResult> = reader.categoryReviewFlags(state, storeId, assignee).map { it.toResult(reader.findStore(it.storeId)?.name) }
    ...
    fun auditEvents(id: UUID): List<AuditEventResult> = reader.auditEvents(id).map { AuditEventResult(it.action, it.resourceId, it.actor, it.createdAt) }

    fun dashboard(): AdminDashboardResult {
        val storeCounts = StoreStatus.entries.associate { it.name to reader.storesByStatus(it).size }
        val feedbackCounts = FeedbackResolutionStatus.entries.associate { status -> status.name to reader.feedback().count { it.resolutionStatus == status } }
        val categoryHealth = categoryHealth()
        return AdminDashboardResult(storeCounts, feedbackCounts, reader.correctionRules().count { it.active }, categoryHealth, categoryReviewFlags(CategoryReviewFlagState.OPEN).size)
    }

    fun categoryHealth(): List<CategoryHealthResult> = Category.entries.map { category ->
        ...
        CategoryHealthResult(
            ...
            reader.categoryReviewFlags(CategoryReviewFlagState.OPEN).count { flag -> reader.findStore(flag.storeId)?.categories?.contains(category) == true },
        )
    }

    private fun StoreResult.matches(query: String): Boolean {
        val normalized = normalize(query)
        return listOf(name, address, description.orEmpty(), *categories.map { it.name }.toTypedArray()).any { normalize(it).contains(normalized) }
    }
    ...
    private fun StoreResult.distanceTo(near: Coordinates): Double {
        ...
        val a = sin(latitudeDelta / 2).pow(2) + cos(Math.toRadians(near.latitude.toDouble())) * cos(Math.toRadians(coordinates.latitude.toDouble())) * sin(longitudeDelta / 2).pow(2)
    ...
    private fun decodeCursor(cursor: String?): Int = cursor?.let { runCatching { Base64.getUrlDecoder().decode(it).decodeToString().toInt() }.getOrElse { throw IllegalArgumentException("invalid cursor") } } ?: 0
