...
class AdminOperationsController(private val catalog: CatalogService) {
    @GetMapping("/correction-rules")
    fun correctionRules(): ApiResponse<List<CorrectionRuleResponse>> = ApiResponse.success(catalog.correctionRules().map { it.toResponse() })

    @PostMapping("/correction-rules")
    @ResponseStatus(HttpStatus.CREATED)
    fun createCorrectionRule(@RequestBody request: CorrectionRuleRequest, principal: Principal): ApiResponse<CorrectionRuleResponse> =
    ...
    fun updateCorrectionRule(@PathVariable ruleId: UUID, @RequestBody request: CorrectionRuleRequest, principal: Principal): ApiResponse<CorrectionRuleResponse> =
        ApiResponse.success(
    ...
    fun deleteCorrectionRule(@PathVariable ruleId: UUID, principal: Principal): ApiResponse<Nothing> {
        if (!catalog.deleteCorrectionRule(ruleId, principal.name)) throw ResponseStatusException(HttpStatus.NOT_FOUND, "correction rule not found")
