...
class AdminStoreController(
    ...
    fun create(
        ...
        principal: Principal,
    ): ApiResponse<AdminStoreResponse> = ApiResponse.success(catalog.create(request.toCommand(), principal.name).toAdminResponse())
    ...
    fun review(
        @PathVariable storeId: UUID,
        principal: Principal,
    ): ApiResponse<AdminStoreResponse> = ApiResponse.success(transition(storeId, StoreStatus.IN_REVIEW, principal))

    @PostMapping("/{storeId}/publish")
    fun publish(
        @PathVariable storeId: UUID,
        principal: Principal,
    ): ApiResponse<AdminStoreResponse> = ApiResponse.success(transition(storeId, StoreStatus.PUBLISHED, principal))

    @PostMapping("/{storeId}/reject")
    fun reject(
        @PathVariable storeId: UUID,
        @RequestBody(required = false) request: StoreRejectionRequest?,
        principal: Principal,
    ): ApiResponse<AdminStoreResponse> = ApiResponse.success(transition(storeId, StoreStatus.REJECTED, principal, request?.reason))
...
private fun StoreRequest.toCommand() =
    StoreUpsertCommand(name, address, Coordinates(coordinates.latitude, coordinates.longitude), categories, phone, hours, description)

private fun StoreResult.toAdminResponse() =
    AdminStoreResponse(
        id,
        name,
        address,
